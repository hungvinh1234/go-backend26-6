...
func Init(group *echo.Group, accountUsecase account.IUsecase) {
	...
	group.GET("/:id/reflist", r.ShowRefList)
...
func (r *Route) ShowRefList(c echo.Context) error {
	ctx := &util.CustomEchoContext{c}
	...
	accounts, err := r.accountUsecase.ShowRefList(ctx, account)
...
// 	uploads, err := r.accountUsecase.Upload(ctx)
