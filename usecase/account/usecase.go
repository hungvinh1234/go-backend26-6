...
func (e *usecase) EditAccount(ctx context.Context, account *model.Account) (*model.Account, error) {
	...
	if currentUser.ID != account.ID && !currentUser.IsAdmin {
...
func (e *usecase) ShowRefList(ctx context.Context, account *model.Account) (*[]model.Account, error) {
	...
	//err la khong tra ve ket qua mong muon
	if err != nil {
		return nil, util.NewError(err, http.StatusInternalServerError, 1010, "failed get user by id for ref list")
	}

	accountlist, err := e.accountRepo.GetRefList(ctx, accountdetail.Username)
	//err la khong tra ve ket qua mong muon
	if err != nil {
		return nil, util.NewError(err, http.StatusInternalServerError, 1010, "failed get ref list")
	}
...
func (e *usecase) Upload(c echo.Context) error {
